...
def app():
    ...
    st.markdown("<h4 style='text-align: center;'>Data Gempa Bumi Sebelum di Preprocessing</h4>", unsafe_allow_html=True)
    st.dataframe(data)
    st.write("Jumlah Record:", data.shape[0])
    st.write("Jumlah Atribut:", data.shape[1])

   
    if st.button("Dataset Setelah Preprocessing"):
       clean_data = pd.read_csv("data_tsunami_bersih.csv")
       st.markdown("<h4 style='text-align: center;'>Data Gempa Bumi Setelah Preprocessing</h4>", unsafe_allow_html=True)
